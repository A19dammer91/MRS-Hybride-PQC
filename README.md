...
> *Defines the exact window of valid alibi transformations via Popoviciu's cardinality formula before the parameters underflow, mathematically establishing the boundaries of the alibi forest. The total number of representations at a layer is $R(N) = K_{max} + 1$.*
...
> *Only representations satisfying this digital-root triangle condition are admitted into the weighted CDF sampling pool. The closed-form count is computed in $O(1)$ via modular arithmetic on the step index $k$.*
...
$$k_{\text{max}} = \left\lfloor \frac{b_0}{19} \right\rfloor$$

$$t_{\text{max}} = \left\lfloor \frac{k_{\text{max}} - k_0}{9} \right\rfloor$$

$$t \sim \mathcal{U}[0, t_{\text{max}}] \quad \text{(rejection-free CSPRNG)}$$

$$k = k_0 + 9t$$

$$a = a_0 + 9k \qquad b = b_0 - 19k$$

> *Reconstructs the exact $(a, b)$ pair for the current Matryoshka layer in $O(1)$ without enumeration. For the 3-layer `sample_three_layers` engine this loop repeats with $N \leftarrow a$ until depth is reached.*
...
### 🎭 Alibi Engine — Mathematical Foundation

When a user is coerced, the `security::alibi` module forges a mathematically valid alternative credential. The LWE secret is derived by solving the public linear system exactly over the prime field $\mathbb{Z}_q$:

$$A \cdot s_{\text{alibi}} \equiv b - e_{\text{alibi}} \pmod{q}$$

> *Because $q$ is prime and $A$ is a public matrix, there is always a unique valid solution for $s_{\text{alibi}}$. When `verify_lwe_match` is called with this fabricated $s_{\text{alibi}}$, the equation matches mathematically perfect and the noise is guaranteed acceptable.*
